class TriangularParticle {
  constructor(asteroidX, asteroidY, isPointedUp, usedSide) {
    ...
    this.p2 = { x: this.x + 12.5, y: isPointedUp ? this.y - this.shapeHeight : this.y + this.shapeHeight }
    ...
    switch (this.usedSide) {
      case 0:
        this.emptyEdges.push({ x: this.p1Connect.x, y: this.p1Connect.y, isPointedUp: !isPointedUp, usedSide: 1 })
        this.emptyEdges.push({ x: this.p2Connect.x, y: this.p2Connect.y, isPointedUp: !isPointedUp, usedSide: 2 })
        this.emptyEdges.push({ x: this.p3Connect.x, y: this.p3Connect.y, isPointedUp: !isPointedUp, usedSide: 3 })
        break;
      case 1:
        this.emptyEdges.push({ x: this.p3Connect.x, y: this.p3Connect.y, isPointedUp: !isPointedUp, usedSide: 3 })
        this.emptyEdges.push({ x: this.p2Connect.x, y: this.p2Connect.y, isPointedUp: !isPointedUp, usedSide: 2 })
